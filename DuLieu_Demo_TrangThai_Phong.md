...
| `PHONG_3` | `trong` | Hồ sơ `moi`, lịch `sap_den` | Chờ xác nhận đã xem | Sale | Xác nhận → lịch `hoan_thanh`, hồ sơ `da_xem_phong` |
| `PHONG_4` | `trong` | Hồ sơ `da_xem_phong`, lịch `hoan_thanh` | Khách đã xem phòng | Khách hàng | Gửi yêu cầu cọc → hồ sơ `cho_ra_soat_coc` |
...
|---|---|---|---|---|---|---|
| `PHONG_5` | `trong` | `da_xem_phong` | Chưa có | Đã xem nhưng chưa yêu cầu cọc | Khách hàng | Gửi yêu cầu → hồ sơ `cho_ra_soat_coc` |
| `PHONG_6` | `trong` | `cho_ra_soat_coc` | Chưa có | Yêu cầu cọc mới | Sale | Rà soát thông tin khách/người ở và chuyển Quản lý |
| `PHONG_7` | `trong` | `cho_ra_soat_coc` | Chưa có | Sale đã rà soát, chờ kiểm tra chỗ trống | Quản lý | Xác nhận phòng còn trống → cho phép lập phiếu cọc |
| `PHONG_8` | `trong` | `cho_ra_soat_coc` | `cho_thanh_toan`, chưa có minh chứng | Phiếu cọc đã lập | Khách hàng | Thanh toán và tải minh chứng |
| `PHONG_9` | `trong` | `cho_ra_soat_coc` | `cho_thanh_toan`, đã có minh chứng | Khách đã thanh toán | Kế toán | Đối chiếu → phiếu `hoan_thanh`, hồ sơ `da_dat_coc`, phòng `da_dat_coc` |
...
|---|---|---|---|---|---|---|
| `PHONG_11` | `da_dat_coc` | `da_dat_coc` | Chưa có hợp đồng | Chưa bổ sung hồ sơ nhận phòng | Khách hàng | Bổ sung danh sách người ở → `cho_kiem_tra_nhan_phong` |
| `PHONG_12` | `da_dat_coc` | `cho_kiem_tra_nhan_phong` | Chưa có | Hồ sơ vừa được gửi | Sale | Đối chiếu giấy tờ, phiếu cọc và chuyển Quản lý |
| `PHONG_13` | `da_dat_coc` | `cho_kiem_tra_nhan_phong` | Chưa có | Chờ duyệt điều kiện lưu trú | Quản lý | Duyệt → hồ sơ `du_dieu_kien_nhan_phong` |
...
| `PHONG_18` | `dang_thue` | Đã hoàn tất | Hợp đồng `hieu_luc`, hóa đơn `da_thanh_toan` | Đang thuê bình thường | Khách hàng | Sử dụng phòng hoặc bắt đầu UC4 khi muốn trả |
...
|---|---|---|---|---|
| `PHONG_37` | `so_tien_hoan > 0` | Hóa đơn `hoan_coc`, chứng từ chi | Kế toán | Chuyển tiền cho khách → hóa đơn `da_thanh_toan`, hợp đồng `thanh_ly`, phòng `trong` |
| `PHONG_38` | `so_tien_thu_them > 0` | Hóa đơn `thu_them = cho_thanh_toan` | Khách hàng | Thanh toán khoản phát sinh và tải minh chứng |
| `PHONG_39` | Khách đã trả khoản thu thêm | Hóa đơn `thu_them = cho_thanh_toan` + minh chứng | Kế toán | Xác nhận tiền → hóa đơn `da_thanh_toan`, hợp đồng `thanh_ly`, phòng `trong` |
| `PHONG_40` | Không hoàn, không thu thêm | Không phát sinh giao dịch | Kế toán | Xác nhận hoàn tất → hợp đồng `thanh_ly`, phòng `trong` |
